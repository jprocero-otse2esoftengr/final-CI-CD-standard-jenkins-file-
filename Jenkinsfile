            ...
            steps {
                dir('.') {
                    bat """
                        ...
                        echo Checking if service exists and stopping if necessary...
                        npx e2e-bridge-cli list -h ${params.BRIDGE_HOST} -u ${params.BRIDGE_USER} -P ${params.BRIDGE_PASSWORD} | findstr regtestlatest >nul
                        if errorlevel 1 (
                            echo Service not found, proceeding with deployment...
                        ) else (
                            echo Service found, stopping it first...
                            npx e2e-bridge-cli stop regtestlatest -h ${params.BRIDGE_HOST} -u ${params.BRIDGE_USER} -P ${params.BRIDGE_PASSWORD}
                        )
                        echo Now deploying with overwrite option...
                        ...
                        echo Starting the service with control port configuration...
                        npx e2e-bridge-cli start regtestlatest -h ${params.BRIDGE_HOST} -u ${params.BRIDGE_USER} -P ${params.BRIDGE_PASSWORD} --controlport ${params.CONTROL_PORT}
                        ...
                        echo Service should now be ready with control port ${params.CONTROL_PORT}
